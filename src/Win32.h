...
#undef X
		...
		const char* className = "CyberSeaquell";
		windowClass.lpszClassName = className;
		if (!pRegisterClassExA(&windowClass)) {
			abort("RegisterClassExA failed");
		}
		window = pCreateWindowExA(0, className, "Cyber Seaquell", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, int(width), int(height), NULL, NULL, instance, NULL);
		if (!window) {
			success = false;
		} else {
			windowWidth = width;
