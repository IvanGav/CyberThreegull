	...
	MEMORY_ARENA_FRAME(stackArena) {
		...
		if (findEntry != INVALID_HANDLE_VALUE) {
			do {
				...
				StrA ext{};
				if (fileName.ends_with(".vert"a)) {
					ext = ".vspv"a;
				} else if (fileName.ends_with(".frag"a)) {
					ext = ".fspv"a;
				}
				if (ext.is_empty()) {
					continue;
				...
				StrA programToRun = stracat(stackArena, outputDir, "glslc.exe"a);
				StrA programArgs = stracat(stackArena, "\""a, shaderDir, fileName, "\" -g -O -o \""a, outputPath, "\" --target-env=vulkan1.2"a);
				println(stracat(stackArena, "Recompiling: "a, shaderDir, fileName));
				U32 exitCode;
...
void init_vulkan() {
	recompile_modified_shaders(".\\resources\\shaders\\"a, ".\\resources\\shaders\\spirv\\"a);
...
void load_pipelines_and_descriptors() {
	...
	uiPipeline = GraphicsPipelineBuilder{}.set_default()
		...
		.vertex_shader("./resources/shaders/spirv/ui.vspv"a)
		.fragment_shader("./resources/shaders/spirv/ui.fspv"a)
		.blending(VK_BLEND_FACTOR_SRC_ALPHA, VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA)
		.build();

	basicPipelineLayout = PipelineLayoutBuilder{}.set_default()
		.push_constant(VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(BasicPipelineRenderData))
		.set_layout(drawDataDescriptorSet.setLayout)
		.build();
	basicPipeline = GraphicsPipelineBuilder{}.set_default()
		.layout(basicPipelineLayout)
		.vertex_shader("./resources/shaders/spirv/basic.vspv"a)
		.fragment_shader("./resources/shaders/spirv/basic.fspv"a)
		.blending(VK_BLEND_FACTOR_SRC_ALPHA, VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA)
		.build();
}
