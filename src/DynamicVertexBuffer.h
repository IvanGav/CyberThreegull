...
struct Tessellator {
	...
	Tessellator& basic_circle(V2F32 pos, F32 radius, U32 segments, V4F32 color, U32 texIdx) {
		...
		MEMORY_ARENA_FRAME(arena) {
			VK::BasicVertex* vertices = arena.alloc<VK::BasicVertex>(segments + 1);
			vertices[0] = VK::BasicVertex{ V3F32{ pos.x, pos.y, z }, V2F32{ 0.5F, 0.5F }, packedColor, texIdx };

			U32 indexOffset = currentIndexByteCount / sizeof(U32);
			F32 segmentStep = 1.0F / F32(segments);
			V2F32 lastPos{ radius, 0.0F };
			for (U32 i = 0; i < segments; i++) {
				F32 y;
				F32 x = sincosf32(&y, i * segmentStep);
				vertices[i + 1] = VK::BasicVertex{ V3F32{ pos.x + x * radius, pos.y + y * radius, z }, V2F32{ x * 0.5F + 0.5F, y * 0.5F + 0.5F }, packedColor, texIdx };
				indexDataPointer[indexOffset + 0] = cmd.vertexCount + 0;
				indexDataPointer[indexOffset + 2] = cmd.vertexCount + 1 + i;
				indexDataPointer[indexOffset + 1] = cmd.vertexCount + 1 + (i + 1) % segments;
				indexOffset += 3;
			}

			memcpy(vertexDataPointer + currentVertexByteCount, vertices, (segments + 1) * sizeof(VK::UIVertex));
			
			currentVertexByteCount += (segments + 1) * sizeof(VK::BasicVertex);
			currentIndexByteCount += segments * 3 * sizeof(U32);
			cmd.vertexCount += segments + 1;
			cmd.indexCount += segments * 3;
		}
		return *this;
	}
