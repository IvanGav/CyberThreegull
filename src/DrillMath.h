...
// Returns qnan if no intersection, otherwise first intersection time
FINLINE F32 ray_intersect_circle(V2F32 circleCenter, F32 radius, V2F32 rayOrigin, V2F32 rayDirection) {
	V2F32 correctedRayOrigin = rayOrigin - circleCenter;
	F32 a = dot(rayDirection, rayDirection);
	F32 b = 2.0F * dot(rayDirection, correctedRayOrigin);
	F32 c = dot(correctedRayOrigin, correctedRayOrigin) - radius * radius;
